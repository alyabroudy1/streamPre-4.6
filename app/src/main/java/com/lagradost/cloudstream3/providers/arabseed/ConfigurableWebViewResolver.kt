...
class ConfigurableCloudflareKiller(
    ...
    private suspend fun bypassCloudflare(request: Request): Response? {
        ...
        ConfigurableWebViewResolver(
            ...
            // Detect any redirect to a different domain (except Cloudflare infrastructure)
            if (reqHost != null && 
        ...
        if (extractedCookies.isEmpty()) {
            ...
            // Try CF proxy domain (a.asd.homes etc) if we detected one during navigation
            if (extractedCookies.isEmpty() && finalDomain != originalHost && !finalDomain.contains("arabseed") && !finalDomain.contains("fasel")) {
                // finalDomain might be a CF proxy like a.asd.homes - try to get cookies from there
                val proxyUrl = "https://$finalDomain"
                val proxyCookies = getWebViewCookie(proxyUrl)
                Log.i(CF_FLOW_TAG, ">>> [COOKIE_MANAGER] CF Proxy URL: $proxyUrl")
                Log.i(CF_FLOW_TAG, ">>> [COOKIE_MANAGER] CF Proxy Cookies: ${proxyCookies?.take(100)}")
                
                if (!proxyCookies.isNullOrBlank()) {
                    extractedCookies = parseCookieMap(proxyCookies)
                    // Store cookies for BOTH original and proxy domains
                    savedCookies[finalDomain] = extractedCookies
                    Log.i(CF_FLOW_TAG, ">>> [SAVED] Cookies also saved for CF proxy domain: $finalDomain")
                }
            }
        }
