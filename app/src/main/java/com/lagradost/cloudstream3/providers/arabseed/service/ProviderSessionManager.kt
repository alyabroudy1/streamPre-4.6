...
import com.lagradost.cloudstream3.providers.arabseed.utils.DomainConfigManager
import com.lagradost.cloudstream3.providers.arabseed.utils.CloudflareChallengeDialog
import com.lagradost.cloudstream3.providers.arabseed.service.cloudflare.CloudflareDetector
import com.lagradost.cloudstream3.providers.arabseed.service.cookie.CookieLifecycleManager
import com.lagradost.cloudstream3.providers.arabseed.service.strategy.*
    ...
    suspend fun request(url: String): StrategyResponse {
        ...
        if (directHttpStrategy.canHandle(RequestContext(url, hasValidCookies = true))) {
             ...
             if (response.success) {
                 ...
                 if (!finalUrl.isNullOrEmpty() && finalUrl != url) {
                     domainConfigManager.checkForDomainChange(finalUrl)
                 }
        ...
        return cfMutex.withLock {
            ...
            if (startFresh) {
                ...
                ProviderLogger.i(TAG_SESSION, "request", "Starting CF challenge dialog",
                    "url" to url.take(80)
                )
            } else {
                 updateSessionState(SessionState.SOLVING, "Soft Refresh (Cookies preserved)")
                 ProviderLogger.i(TAG_SESSION, "request", "Starting CF challenge dialog (Cookies preserved)",
                    "url" to url.take(80)
                )
            }
            
            // Use CloudflareChallengeDialog for interactive CF solving
            val challengeResult = CloudflareChallengeDialog.solve(url, userAgent)
            
            val response = if (challengeResult.success && challengeResult.cookies.isNotEmpty()) {
                // Store new cookies
                cookieManager.store(url, challengeResult.cookies, "Dialog-Fresh")
                stateStore.saveCookies(challengeResult.cookies)
                
                // Check for domain redirect
                if (challengeResult.finalUrl.isNotEmpty() && challengeResult.finalUrl != url) {
                    domainConfigManager.checkForDomainChange(challengeResult.finalUrl)
                }
                
                updateSessionState(SessionState.VALID, "Dialog solve")
                
                ProviderLogger.i(TAG_SESSION, "request", "Dialog solve successful",
                    "cookieCount" to challengeResult.cookies.size
                )
                
                // Now make actual request with cookies
                directHttpStrategy.execute(buildRequest(url))
            } else {
                updateSessionState(SessionState.EXPIRED, "Dialog solve failed: ${challengeResult.reason}")
                ProviderLogger.w(TAG_SESSION, "request", "Dialog solve failed",
                    "reason" to challengeResult.reason
                )
                StrategyResponse.failure(
                    code = -1,
                    error = Exception("CF Dialog failed: ${challengeResult.reason}"),
            ...
            ProviderLogger.logRequestComplete(url, response.responseCode,
                System.currentTimeMillis() - startTime, "Dialog-Fresh")
