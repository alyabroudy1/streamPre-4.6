...
class DomainConfigManager(
    ...
    suspend fun checkForDomainChange(responseUrl: String): Boolean {
        ...
        // Detect ANY domain change (not just provider-specific domains)
        // This handles CF proxy domains like a.asd.homes
        if (newDomain != currentBase && newDomain.isNotEmpty()) {
            ...
            // Only sync to GitHub if it looks like a provider domain (not CF infrastructure)
            if (!newDomain.contains("cloudflare") && !newDomain.contains("cdn-cgi")) {
